document.addEventListener('DOMContentLoaded', function () {
  ...
  },
  {
    ...
    function getUniqueValues(key) {
        const allValues = quotes.flatMap(quote => quote[key]);
        return [...new Set(allValues)].sort();
    }

    function populateFilters() {
        const virtues = getUniqueValues('virtues');
        const philosophers = getUniqueValues('author');
    ...
    function filterQuotes() {
        ...
        currentQuotes = quotes.filter(quote => {
            const virtueMatch = selectedVirtue === 'all' || quote.virtues.includes(selectedVirtue);
            const philosopherMatch = selectedPhilosopher === 'all' || quote.author === selectedPhilosopher;
            return virtueMatch && philosopherMatch;
        });
    ...
    randomQuoteBtn.addEventListener('click', () => displayQuote(getRandomQuote()));
